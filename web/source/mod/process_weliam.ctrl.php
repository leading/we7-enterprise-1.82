...
//if (is_error($prepare)) {
	//itoast($prepare['message'], url('mod/profile_yc'), 'error');
//}
...
DAT;
	...
	if (is_error($packet)) {
		itoast($packet['message'], '', 'error', 'success');
	} else {
		cache_delete('checkupgrade:system');
		cache_delete('cloud:transtoken');
		itoast('更新已完成. ', url('mod/mod_weliam'), 'success');
	}
}
template('mod/process_weliam');